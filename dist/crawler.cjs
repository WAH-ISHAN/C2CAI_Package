...
// src/crawler.ts
async function crawlAndSplit(siteUrl) {
  const { chunkSize, chunkOverlap, maxPages } = config.crawler;
  let allDocs = [];
  try {
    const sitemapRes = await axios_default.get(`${siteUrl}/sitemap.xml`);
    const parser = await (0, import_xml2js.parseStringPromise)(sitemapRes.data);
    const urls = parser.urlset.url.slice(0, maxPages).map((u) => u.loc[0]);
    for (const url2 of urls) {
      const loader = new import_cheerio.CheerioWebBaseLoader(url2, { selector: "p, h1, h2, h3, .content, .product" });
      const docs = await loader.load();
      allDocs.push(...docs);
    }
  } catch (e) {
    console.log("Sitemap not found, crawling main page.");
    const loader = new import_cheerio.CheerioWebBaseLoader(siteUrl);
    const docs = await loader.load();
    allDocs = [docs];
  }
  const splitter = new import_textsplitters.RecursiveCharacterTextSplitter({ chunkSize, chunkOverlap });
